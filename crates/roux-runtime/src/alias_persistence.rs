...
pub fn persistence_path() -> PathBuf {
    roux_core::paths::roux_config_dir().join("aliases.json")
}
...
pub fn load_from_path(path: &Path) -> Vec<AgentAlias> {
    if !path.exists() {
        return Vec::new();
    }
...
pub fn save_to_path(entries: &[AgentAlias], path: &Path) -> std::io::Result<()> {
