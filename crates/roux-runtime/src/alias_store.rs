...
pub enum BindError {
    #[error("alias '{canonical}' is already bound to {current}; pass force=true to override")]
...
impl AliasStore {
    ...
    pub fn bind(&mut self, canonical: &str, req: BindRequest) -> Result<AgentAlias, BindError> {
        let now = now_ms();
    ...
    pub fn get(&self, canonical: &str, project_id: Option<&str>) -> Option<&AgentAlias> {
        self.entries.iter().find(|a| a.alias == canonical && a.project_id.as_deref() == project_id)
    }
    ...
    fn find_mut(&mut self, canonical: &str, project_id: Option<&str>) -> Option<&mut AgentAlias> {
        self.entries
            .iter_mut()
...
mod tests {
    ...
    fn project_scoped_aliases_are_independent() {
        ...
        assert_eq!(store.get("reviewer", None).unwrap().session_id.as_deref(), Some("sess-global"));
    }
    ...
    fn unbind_for_pane_skips_manual_when_only_auto() {
        ...
        assert_eq!(store.get("manual", None).unwrap().pane_id.as_deref(), Some("pane-A"));
    }
    ...
    fn set_consumption_mode_updates_field() {
        let mut store = AliasStore::new();
        store.ensure("team", None);
        store.set_consumption_mode("team", None, ConsumptionMode::Broadcast).unwrap();
        assert_eq!(store.get("team", None).unwrap().consumption_mode, ConsumptionMode::Broadcast,);
    }
    ...
    fn find_for_pane_includes_group_members() {
        ...
        let aliases_for_a: Vec<_> =
            store.find_for_pane("pane-A").iter().map(|a| a.alias.clone()).collect();
