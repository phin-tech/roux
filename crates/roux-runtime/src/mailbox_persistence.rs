...
pub fn load_events_from(path: &Path) -> Vec<Event> {
    if !path.exists() {
        return Vec::new();
    }
...
pub fn append_event_to(path: &Path, event: &Event) -> io::Result<()> {
...
pub fn append_retract_to(path: &Path, event_id: &str, retracted_at: u64) -> io::Result<()> {
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent)?;
    }
...
pub fn load_read_state_from(path: &Path) -> Vec<ReadState> {
    if !path.exists() {
        return Vec::new();
    }
...
pub fn save_read_state_to(path: &Path, states: &[ReadState]) -> io::Result<()> {
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent)?;
    }
