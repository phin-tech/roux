...
use crate::mailbox_persistence::{
    append_event_to, append_retract_to, events_path, load_events_from, load_read_state_from,
    read_state_path, save_read_state_to,
};
use crate::mailbox_store::{EventStore, PostError, RetractError};
use crate::subscription_service::SubscriptionManager;
...
impl MailboxManager {
    ...
    pub fn load() -> Self {
        Self::load_from(events_path(), read_state_path())
    }
    ...
    pub fn post(&self, builder: EventBuilder) -> Result<Event, PostError> {
    ...
    pub fn mark_read(&self, event_id: &str, recipient: &str) -> bool {
        let now = now_ms();
        ...
        let patterns =
            self.get(event_id).map(|e| self.patterns_for_event(recipient, &e)).unwrap_or_default();
        let changed = {
            let mut store = self.inner.lock().expect("event store poisoned");
    ...
    pub fn ack(&self, event_id: &str, recipient: &str, result: Option<String>) -> bool {
        let now = now_ms();
        let patterns =
            self.get(event_id).map(|e| self.patterns_for_event(recipient, &e)).unwrap_or_default();
        let changed = {
    ...
    pub fn clear_read(&self, recipient: &str, project_filter: ProjectFilter<'_>) -> usize {
        let now = now_ms();
        let cleared = {
            let mut store = self.inner.lock().expect("event store poisoned");
        ...
        if cleared > 0 {
            self.persist_read_state();
            let evt =
                MailboxEvent::Cleared { recipient: recipient.to_string(), count: cleared as u32 };
            self.broadcast(&evt);
        }
        cleared
    }
    ...
    pub fn retract(&self, event_id: &str, caller: &str) -> Result<Event, RetractError> {
        let now = now_ms();
        let event = {
            let mut store = self.inner.lock().expect("event store poisoned");
    ...
    pub fn dismiss(&self, event_id: &str, recipient: &str) -> bool {
        let now = now_ms();
        let patterns =
            self.get(event_id).map(|e| self.patterns_for_event(recipient, &e)).unwrap_or_default();
        let changed = {
...
mod tests {
    ...
    fn post_persists_to_events_jsonl() {
        ...
        let event = mgr.post(task("hello")).unwrap();
    ...
    fn post_round_trips_through_disk_reload() {
        ...
        mgr1.post(task("first")).unwrap();
        mgr1.post(task("second")).unwrap();
    ...
    fn mark_read_persists_and_survives_reload() {
        ...
        let (e, r) = paths(dir.path());
        let mgr1 = MailboxManager::load_from(e.clone(), r.clone());
        let event = mgr1.post(task("hello")).unwrap();
        assert!(mgr1.mark_read(&event.id, "reviewer"));
    ...
    fn ack_persists_with_result_string() {
        ...
        let mgr1 = MailboxManager::load_from(e.clone(), r.clone());
        let event = mgr1.post(task("hello")).unwrap();
        assert!(mgr1.ack(&event.id, "reviewer", Some("done!".into())));
    ...
    fn clear_read_marks_cleared_and_persists() {
        ...
        let e1 = mgr.post(task("a")).unwrap();
        let e2 = mgr.post(task("b")).unwrap();
        mgr.mark_read(&e1.id, "reviewer");
        mgr.mark_read(&e2.id, "reviewer");

        let cleared = mgr.clear_read("reviewer", ProjectFilter::Any);
        assert_eq!(cleared, 2);
    ...
    fn unread_count_excludes_read() {
        ...
        let e1 = mgr.post(task("a")).unwrap();
        let _e2 = mgr.post(task("b")).unwrap();
    ...
    fn list_all_returns_newest_first() {
        let mgr = MailboxManager::in_memory();
        mgr.post(task("first")).unwrap();
        mgr.post(task("second")).unwrap();
    ...
    fn subscriber_sees_topic_event_in_inbox() {
        let mgr = MailboxManager::in_memory().with_subscriptions(SubscriptionManager::in_memory());
        ...
        subs.subscribe("auditor", "**.completed", None).unwrap();
    ...
    fn unsubscribed_recipient_does_not_see_topic_events() {
        ...
        let topic_event =
            EventBuilder::new("hi").topic("repo-a.build.completed").kind(EventKind::Signal);
        mgr.post(topic_event).unwrap();
    ...
    fn subscriber_unread_count_includes_topic_matches() {
        let mgr = MailboxManager::in_memory().with_subscriptions(SubscriptionManager::in_memory());
        mgr.subscriptions.as_ref().unwrap().subscribe("auditor", "**.completed", None).unwrap();

        let topic_event = EventBuilder::new("hi").topic("build.completed").kind(EventKind::Signal);
        let event = mgr.post(topic_event).unwrap();

        assert_eq!(mgr.unread_count("auditor", ProjectFilter::Any), 1);
        // Auditor reads the topic event via the manager (subscription
        // ownership lets them).
        assert!(mgr.mark_read(&event.id, "auditor"));
    ...
    fn project_scoped_subscription_only_matches_in_scope() {
        let mgr = MailboxManager::in_memory().with_subscriptions(SubscriptionManager::in_memory());
        mgr.subscriptions.as_ref().unwrap().subscribe("auditor", "*", Some("p1".into())).unwrap();

        // Event in p1: matches.
        let in_p1 = EventBuilder::new("a").topic("foo").project_id("p1").kind(EventKind::Signal);
        mgr.post(in_p1).unwrap();

        // Event in p2: must not match.
        let in_p2 = EventBuilder::new("b").topic("foo").project_id("p2").kind(EventKind::Signal);
        mgr.post(in_p2).unwrap();

        let visible_p1 = mgr.list_for_recipient("auditor", false, ProjectFilter::Exact(Some("p1")));
        assert_eq!(visible_p1.len(), 1);

        let visible_p2 = mgr.list_for_recipient("auditor", false, ProjectFilter::Exact(Some("p2")));
    ...
    fn cross_project_subscription_cannot_authorize_other_project_events() {
        ...
        mgr.subscriptions.as_ref().unwrap().subscribe("auditor", "*", Some("p2".into())).unwrap();
    ...
    async fn broadcast_fires_topic_delivered_for_each_subscription() {
        let mgr = MailboxManager::in_memory().with_subscriptions(SubscriptionManager::in_memory());
        mgr.subscriptions.as_ref().unwrap().subscribe("auditor", "**.completed", None).unwrap();
        mgr.subscriptions.as_ref().unwrap().subscribe("watcher", "**", None).unwrap();

        let mut rx = mgr.subscribe_events();
        let topic_event = EventBuilder::new("a").topic("build.completed").kind(EventKind::Signal);
        mgr.post(topic_event).unwrap();
        ...
        assert_eq!(delivered_recipients, vec!["auditor".to_string(), "watcher".to_string()]);
    }

    #[tokio::test]
    async fn broadcast_fires_on_mark_read_and_ack() {
        let mgr = MailboxManager::in_memory();
        let event = mgr.post(task("hi")).unwrap();

        let mut rx = mgr.subscribe_events();
        mgr.mark_read(&event.id, "reviewer");
        mgr.ack(&event.id, "reviewer", Some("done".into()));
    ...
    fn retract_round_trips_through_disk_reload() {
        ...
        let event = mgr.post(task("ephemeral")).unwrap();
        let retracted = mgr.retract(&event.id, "me").unwrap();
    ...
    async fn retract_broadcasts_retracted_event() {
        let mgr = MailboxManager::in_memory();
        let event = mgr.post(task("oops")).unwrap();
        let mut rx = mgr.subscribe_events();
        mgr.retract(&event.id, "me").unwrap();
    ...
    fn dismiss_round_trips_through_disk_reload() {
        ...
        let (e, r) = paths(dir.path());
        let mgr = MailboxManager::load_from(e.clone(), r.clone());
        let event = mgr.post(task("noisy")).unwrap();
        assert!(mgr.dismiss(&event.id, "reviewer"));

        let mgr2 = MailboxManager::load_from(e, r);
        let inbox = mgr2.list_for_recipient("reviewer", false, ProjectFilter::Any);
        assert!(inbox.is_empty(), "dismissed event must not reappear after reload");
    }

    #[tokio::test]
    async fn dismiss_broadcasts_dismissed_event() {
        let mgr = MailboxManager::in_memory();
        let event = mgr.post(task("noise")).unwrap();
        let mut rx = mgr.subscribe_events();
        mgr.dismiss(&event.id, "reviewer");
        let frame = rx.recv().await.unwrap();
        match frame {
    ...
    fn retract_rejected_after_ack() {
        let mgr = MailboxManager::in_memory();
        let event = mgr.post(task("x")).unwrap();
        mgr.ack(&event.id, "reviewer", Some("done".into()));
        let err = mgr.retract(&event.id, "me").unwrap_err();
