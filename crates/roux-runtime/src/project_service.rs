...
pub fn spawn_with_path(
    ...
    let (handle, future) = service_with_path(initial_projects, persist_path);
...
pub fn service_with_path(
    initial_projects: Vec<Project>,
    persist_path: PathBuf,
) -> (ProjectHandle, impl std::future::Future<Output = ()> + Send + 'static) {
    let (tx, rx) = mpsc::unbounded_channel();
    (ProjectHandle { tx }, service_loop(rx, initial_projects, persist_path))
}
