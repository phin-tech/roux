...
- pane layout socket commands such as split, shell, and focus still expect the desktop app to be running
...
- `roux session panes list|create` work against daemon-owned PTYs when the daemon owns the socket, but create does not mutate GUI layout
