...
Roux.app has thin Tauri command wrappers for the same process registry, PTY registry, session lifecycle, worktree command surface, durable watch state, alias state, mailbox state, bus subscription state, and notes vault commands. When an external daemon is connected, those wrappers forward to the daemon socket; when the desktop is self-hosting, they use the embedded runtime host or local filesystem helpers. Worktree create/remove automation hooks run wherever the worktree operation runs. The desktop-side boundary is now "frontend command adapter" instead of direct process ownership for these paths.
