...
impl VaultPath {
    ...
    pub(crate) fn repo_dir(&self, slug: &str) -> PathBuf {
        self.root.join("repos").join(slug)
    }

    pub(crate) fn project_dir(&self, slug: &str) -> PathBuf {
        self.root.join("projects").join(slug)
    }

    pub(crate) fn session_dir(&self, slug: &str) -> PathBuf {
    ...
    pub(crate) fn scope_dir(&self, scope: &Scope, session_slug: &str) -> PathBuf {
        match scope {
    ...
    pub(crate) fn notes_file(&self, scope: &Scope, topic: Option<&str>, session_slug: &str) -> PathBuf {
...
impl NotesIndex {
    ...
    pub(crate) fn load(vault_root: &Path) -> Self {
    ...
    pub(crate) fn resolve_repo(&mut self, repo_path: &str, remote: Option<&str>) -> String {
    ...
    pub(crate) fn resolve_project(&mut self, project_id: &str, project_name: &str) -> String {
    ...
    fn persist_projects(&self) {
        write_json_map(
            &self.vault_root.join(".roux").join("projects.json"),
            &self.projects,
        );
    }
...
impl NotesService {
    pub(crate) fn new(vault_root: impl Into<PathBuf>) -> Self {
    ...
    pub(crate) fn resolve_target(
        ...
        let scope = match scope_name {
            ...
            "project" => {
                ...
                let project_id = s.project_id.as_deref().ok_or_else(|| {
                    "no project assigned to this session".to_string()
                })?;
                let name = project_name
                    .ok_or_else(|| "project name not provided".to_string())?;
    ...
    pub(crate) fn dir_path(&self, scope: &Scope, session_slug: &str) -> PathBuf {
    ...
    pub(crate) fn append_file(
        ...
        let path = self
            .vault
            .notes_file(scope, topic, session_slug);
        if let Some(parent) = path.parent() {
...
fn tag_matches_prefix(haystack: &str, needle: &str) -> bool {
    haystack == needle
        || (haystack.starts_with(needle)
            && haystack[needle.len()..].starts_with('/'))
}
...
pub(crate) fn session_slug(branch: &str, session_id: &str) -> String {
...
pub(crate) mod slug {
    ...
    pub(crate) fn slugify_path_basename(path: &str) -> Option<String> {
    ...
    pub(crate) fn slugify_remote_url(url: &str) -> Option<String> {
...
pub(crate) mod frontmatter {
    ...
    pub(crate) fn split(contents: &str) -> (Option<&str>, &str) {
...
pub(crate) mod timestamped_entry {
...
mod tests {
    ...
    fn frontmatter_ensure_preserves_unknown_user_fields() {
        ...
        let list: Vec<&str> = map["custom_list"]
            .as_sequence()
            .unwrap()
            .iter()
    ...
    fn frontmatter_ensure_writes_repo_scope_fields() {
        ...
        assert_eq!(
            map["repo_path"].as_str(),
            Some("/Users/sam/src/github.com/phin-tech/roux")
        );
        assert_eq!(
            map["remote"].as_str(),
    ...
    fn index_resolve_repo_uses_remote_slug_and_is_stable() {
        ...
        let s1 = idx.resolve_repo(
            "/Users/sam/src/roux",
            Some("git@github.com:phin-tech/roux.git"),
        );
        assert_eq!(s1, "phin-tech-roux");
        let s2 = idx.resolve_repo(
            "/Users/sam/src/roux",
            Some("git@github.com:phin-tech/roux.git"),
        );
    ...
    fn resolve_target_global_ignores_session_context() {
        ...
        let (scope, topic, _session_slug) = svc
            .resolve_target("global", None, None, None, Some("my-topic".to_string()))
            .unwrap();
    ...
    fn search_multi_tag_is_and() {
        ...
        let hits =
            search_by_tags(tmp.path(), None, &["api".to_string(), "tls".to_string()], false);
    ...
    fn vault_path_builds_notes_files_for_each_scope() {
        ...
        assert_eq!(
            v.notes_file(&repo, None, "irrelevant"),
    ...
    fn frontmatter_ensure_writes_session_scope_fields() {
        ...
        assert_eq!(
            map["worktree"].as_str(),
            Some("/Users/sam/src/worktrees/session-notes")
        );
    }
