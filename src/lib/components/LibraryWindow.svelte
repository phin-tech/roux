...
{#if visible}
  ...
  >
    ...
    >
      <aside
        class="flex w-[min(320px,34vw)] min-w-[240px] shrink-0 flex-col border-r border-hairline bg-bg-surface/30 py-3"
      >
        <div class="border-b border-hairline px-3 pb-3">
          <div class="flex items-center gap-2">
            ...
            <div class="min-w-0">
              <div
                id="library-window-title"
                class="text-[11px] font-semibold uppercase tracking-widest text-text-muted"
              >
                Library Manager
              </div>
              <div class="mt-1 text-[13px] font-semibold text-text-primary">
                Library Items
              </div>
            </div>
          </div>
          <div class="mt-3 flex gap-1">
            <button
              type="button"
              class="flex-1 rounded-md border border-border-subtle bg-bg-surface px-2 py-1.5 text-xs font-semibold text-text-secondary hover:bg-bg-hover hover:text-text-primary"
              onclick={() => newItem("prompt")}>+ Prompt</button
            >
            <button
              type="button"
              class="flex-1 rounded-md border border-border-subtle bg-bg-surface px-2 py-1.5 text-xs font-semibold text-text-secondary hover:bg-bg-hover hover:text-text-primary"
              onclick={() => newItem("skill")}>+ Skill</button
            >
          </div>
          <input
          ...
          <div class="mt-2 grid grid-cols-3 border border-border-subtle">
            {#each ["all", "prompt", "skill"] as type}
              <button
                type="button"
                class="px-2 py-1.5 text-[11px] font-medium uppercase tracking-[0.16em] {typeFilter ===
                type
        ...
        <div class="app-scrollbar min-h-0 flex-1 overflow-y-auto p-2">
          ...
          {:else}
            {#each filteredItems as item (item.id)}
              <button
                type="button"
                class="mb-1 flex w-full cursor-pointer items-start gap-2 px-2 py-2 text-left transition-colors {selected
                  ?.item.id === item.id
                  ? 'bg-bg-active'
                  : 'hover:bg-bg-hover'}"
              ...
              >
                <span
                  class="mt-0.5 shrink-0 border border-border-subtle px-1.5 py-0.5 text-[9px] uppercase tracking-[0.14em] text-text-secondary"
                  >{item.itemType}</span
                >
                <span class="min-w-0 flex-1">
                  <span
                    class="block truncate text-[13px] font-semibold text-text-primary"
                    >{item.title}</span
                  >
                  <span
      ...
      <section class="flex min-w-0 flex-1 flex-col bg-bg-deep">
        <div
          class="flex h-12 shrink-0 items-center justify-between border-b border-hairline px-4"
        >
          <div class="min-w-0">
            <div class="truncate text-sm font-semibold text-text-primary">
              {$libraryWindow.mode === "new"
                ? `New ${$libraryWindow.itemType}`
                : selected
                  ? `Editing ${selected.item.title}`
                  : "Select an item"}
            </div>
            <div class="mt-0.5 truncate font-mono text-[10px] text-text-muted">
              {selected?.item.sourcePath ??
                "Choose a source and save to write a Library markdown file"}
            </div>
          </div>
          <button
            type="button"
            class="rounded-lg border border-border-subtle bg-bg-surface px-3 py-1.5 text-xs text-text-secondary hover:bg-bg-hover hover:text-text-primary"
        ...
        {#if error}
          <div
            class="mx-4 mt-3 border border-red/30 bg-red/10 px-3 py-2 text-xs text-red"
          >
            {error}
          </div>
        {/if}
        ...
        <div class="app-scrollbar min-h-0 flex-1 overflow-y-auto p-4">
          ...
          {:else if $libraryWindow.mode === "browse"}
            <div
              class="flex h-full min-h-[360px] items-center justify-center text-sm text-text-muted"
            >
              Select a library item, or create a new one.
            </div>
          {:else}
            <div
              class="flex h-full min-h-[360px] items-center justify-center text-sm text-text-muted"
