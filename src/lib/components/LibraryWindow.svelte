      ...
      <aside class="flex w-[min(320px,34vw)] min-w-[240px] shrink-0 flex-col border-r border-hairline bg-bg-surface/30 py-3">
        <div class="border-b border-hairline px-3 pb-3">
          ...
          <div class="mt-3 flex gap-1">
            ...
            <button type="button" class="flex-1 rounded-md border border-border-subtle bg-bg-surface px-2 py-1.5 text-xs font-semibold text-text-secondary hover:bg-bg-hover hover:text-text-primary" title="New smolvm install script" onclick={() => newItem("smolvmScript")}>+ Smolvm</button>
          </div>
          <input
            ...
            bind:value={filter}
          />
          <div class="mt-2 grid grid-cols-4 border border-border-subtle">
            {#each ["all", "prompt", "skill", "smolvmScript"] as type}
              <button
                type="button"
